(function(){
  ...
  let mode="live",shiftBaseMs=0,shiftStartedMs=0,playbackTimer=null,recoveryCount=0,renderedBlockId="";
  ...
  function renderGuide(){if(!schedule.length)return;els.guide.innerHTML=schedule.map(b=>`<article class="guide-row guide-card" data-id="${b.id}" style="--card-art:url(\'${art(b.movie)}\')"><div><time>${fmt(b.startsAtMs)}</time><strong>${b.movie.title}</strong><span>${b.movie.collection}</span></div></article>`).join("");}
  ...
  function sync(force){
    ...
    els.title.textContent=block.movie.title;els.meta.textContent=block.movie.host?`HOSTED BY ${block.movie.host.toUpperCase()} · ${block.movie.collection}`:block.movie.collection;els.time.textContent=`${fmt(block.startsAtMs)}–${fmt(block.endsAtMs)}`;updateArt(block.movie);if(renderedBlockId!==block.id){renderedBlockId=block.id;renderNext(block);}
    ...
    if(force||loadedKey!==key){showStation("Tuning USA — starting the scheduled program");loadedKey=key;player.loadVideoById({videoId:state.activeVideoId,startSeconds:state.mediaSeconds});player.setVolume(100);verifyPlayback(state.activeVideoId,state.isIntro||block.type==="cartoon");return;}
    if(player.getPlayerState()===YT.PlayerState.PLAYING){showPlayer();if(mode==="live"){const drift=state.mediaSeconds-player.getCurrentTime();if(Math.abs(drift)>5)player.seekTo(state.mediaSeconds,true);}}
  }
  ...
  window.onYouTubeIframeAPIReady=function(){player=new YT.Player("player",{width:"100%",height:"100%",playerVars:{playsinline:1,controls:1,rel:0,enablejsapi:1},events:{onReady:()=>{playerReady=true;if(entered)sync(true);},onStateChange:event=>{if(event.data===YT.PlayerState.PLAYING){clearTimeout(playbackTimer);const state=resolve(currentClock());let duration=0;try{duration=Number(player.getDuration())||0;}catch(_){}const shortFormAllowed=state?.isIntro||state?.block?.type==="cartoon";if(duration&&duration<1200&&!shortFormAllowed)quarantineCurrent("Short clip rejected — switching to a full program");else{recoveryCount=0;showPlayer();}}if(event.data===YT.PlayerState.ENDED){const state=resolve(currentClock());if(state?.isIntro)finishedIntroIds.add(state.activeVideoId);loadedKey="";showStation(state?.block?.type==="cartoon"?"Cartoon Express — loading the next play":"USA station break — returning to the live schedule");sync(true);}},onError:()=>quarantineCurrent("Source unavailable — switching to another full program")}});};
